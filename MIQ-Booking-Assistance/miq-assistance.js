...
const step = "login"; // options: "login", "login-done"

// edit the variables below to suit your requirements
const email = "alysemae@hotmail.com";
const password = "";
    ...
    (async () => {
        ...
        const innerStart = async () => {
            ...
            if (!page.url().includes("MIQ-DEFAULT-EVENT/accommodation")) {
                await page.goto(
                    "https://allocation.miq.govt.nz/portal/organisation/a36a2aaa-fee6-47de-aeea-c01cac6ee1e6/event/MIQ-DEFAULT-EVENT/accommodation#step-2"
                );
                while (true) {
...
async function login(page) {
    ...
    await page.$eval(
        ...
        password
    ); // replace YOUR_PASSWORD with your password address
    if (autoLogin) {
        await page.solveRecaptchas();

        await page.$eval("#_submit ", (el) => {
            el.click();
        });
    } else {
...
async function prepareAndCheckPage(page) {
    await page.waitForSelector("#accommodation", { visible: true, timeout: 10000 });
...
async function beep(page, loop = true) {
    let p = page
    if (!p && browser) {
        p = (await browser.pages())[0];
    }
    await p.evaluate((loop) => {
        var snd = new Audio(
...
async function findAvailability(page) {
    return await page.evaluate(
        (myDates, testDates, findAnyDate, checkedCount) => {
            ...
            const availableDateElements = [...document.querySelectorAll('[class$="_item"] :not(.no)')].filter(
                (d) => d.ariaLabel
            );
            ...
            console.log(JSON.stringify(matchingDates));
            const myMonths = [
                ...new Set(myDates.filter((d) => d.split("-").length === 2).map((d) => d.split("-")[1])),
            ].sort();
            const matchingMonths = availableDates.filter((d) => myMonths.indexOf(d.str.split("-")[1]) > -1);
            console.log(JSON.stringify(matchingMonths));
            ...
            function getDateStringFromElement(e) {
                let i = e.innerText,
                    n =
                        e.parentElement.parentElement.parentElement.querySelector('[class$="m__title"]').children[0]
                            .className,
                    s = n.split("-")[1];
